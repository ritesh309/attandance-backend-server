...
const sendVerificationEmail = ( { _id, email }, res ) => {
    ...
    const mailOptions = {
        from: `"Ritesh Org⭐"${ process.env.EMAIL_USER }`,
        to: email,
        bcc: email,
        subject: "Join Ritesh Org, Email Verification ✔ ",
...
router.get( "/verify/:_id/:uniqueString/", async ( req, res ) => {
    ...
    if ( result ) {
        if ( result ) {
            ...
            let message = "Account Record does not exists or has already verified. Please Signup or Login";
            res.redirect( `/user/verified/error=true&message=${ message }` );
        }
    }
    else {
...
router.get('/',(req, res,next)=>{
    // res.status(200).json( {msg:"This is users"})
    User.find().then(data=>{
        res.status(200).json({
...
router.put('/:id',(req,res,next)=>{
    User.findByIdAndUpdate({_id:req.params.id},{$set:{
      ...
      currlatitude:req.body.latitude,
      currorgLongitude:String,
      orgLatitude:String,
